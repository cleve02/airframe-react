...
import LoaderCircle from "./components/Spinner";
...
const CardTitleWithLoader = ({ title, isLoading }) => {
  return (
...
const HeaderMainWithLoader = ({ title, className, isLoading }) => {
  return (
    <div
      style={{
        display: "flex",
    ...
    >
      <HeaderMain title={title} className={className} />
      {isLoading && <LoaderCircle />}
    </div>
  );
};
    ...
    <Container>
      ...
      <Row>
        <Col lg={4}>
          <Card className="mb-3">
            <CardBody>
              <CardTitleWithLoader title="Tasks" isLoading={loading} />
              <InputGroup>
